...
- **2.3 ویرایش Hostname و SNI**
  ...
  در این مرحله از ویرایش کد، باید `3` مرتبه `hostname` پیش‌فرض را در لاین‌های `(928)`، `(953)` و `(982)` را با هاست‌نیم کانفیگ خود (مانند UUID) و همچنین `3` مرتبه `SNI` پیش‌فرض در لاین‌های `(931)`، `(956)` و `(985)` را با SNI کانفیگ خود جایگزین کنید. SNI در کانفیگ‌هایی که با وورکر یا پیج ساخته می‌شوند عموماً برابر با Hostname می‌باشد.
