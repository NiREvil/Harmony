...
const fp = ['chrome', 'firefox', 'safari', 'edge', 'chrome', 'ios', 'randomized']; // Preferred fingeprints, is better to use chrome, firefox, safari.

const port = ['8443', '2053', '2086', '2087', '2096']; // Preferred TLS Ports for 1st configs ex: ['443', '8443', '2053', '2083', '2087', '2096'];
...
async function handleRequest(request) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const config = {
      ...defaultConfigvless,
      add: ip,
      ps: '| HARMONY ¹|', // Specify the 1st set configs name
      port: randomport,
      path: randomPath,
    };
  ...
  for (const ip of shuffledIPListRE1) {
    ...
    const config = {
      ...defaultConfigvless,
      add: ip,
      ps: '| HARMONY ²|', // Specify the 2nd set configs name
      path: randomPath,
    };
    const queryParams = new URLSearchParams({
      path: randomPath,
  ...
  for (const ip of shuffledIPListRE2) {
    ...
    const config = {
      ...defaultConfigvless,
      add: ip,
      ps: '| HARMONY ³|', // Specify the 3rd set configs name
      path: randomPath,
    };
    const queryParams = new URLSearchParams({
      path: randomPath,
