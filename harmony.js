/**
 * Last Update: 04:20 UTC - Wednesday, 13 November 2024
 * - Built a v2ray subscription link with the ability to automatically add CF clean IPs to your VLESS configurations.
 * - Replace these lines with your uuid and hostname, uuid in line (50) - hostname in lines (954-957) and (995-998) and (1022-1025). Tamam ;)
 * We are all REvil 
 */
...
const defaultConfigvless = {
  ...
  ps: "",
  port: "443",
  id: "3c36eadb-747a-4166-8db0-23fec9883ea2",// Specify the UUID for the VLESS configuration here.
  aid: "0",
  net: "ws",
  type: "none",
  host: "",
  path: "/REvilxXx?ed=2560",// Preferred path, default set is "/?ed=2048"
  tls: "tls",
  sni: "",
};
...
async function handleRequest(request) {
  ...
  var fp = [// Preferred fingeprints for all configs
      'chrome',
      'firefox',
      'randomized',
      'safari',
  ...
  for (let i = 0; i < 1; i++) {
      ...
      for (let i = 0; i < 10; i++) {
          ...
          const queryParams = new URLSearchParams({
              ...
              host: 'turkish.nscl.ir',// Set your host here -1
              fp: randomfp,
              type: config.net,
              sni: 'TURKISH.NSCL.ir',// Set your host here -2
          });
          const vlessUrl = `vless://${config.id}@${config.add}:${config.port}?${queryParams.toString()}#${config.ps}`;
          configsList.push(vlessUrl);
      }

      const ipv4urlRE1 = 'https://raw.githubusercontent.com/vfarid/cf-clean-ips/main/list.json';// 1st ip sources
      ...
      const ipv4urlRE2 = 'https://raw.githubusercontent.com/NiREvil/Harmony/refs/heads/main/cf-clean.json';// 2nd ip sources
      ...
      for (const ip of shuffledIPListRE1) {
          ...
          const queryParams = new URLSearchParams({
              ...
              encryption: config.type,
              alpn: 'http/1.1',// Preferred alpn type
              host: 'zizifn-vless.pages.dev',// Set your host here -3
              fp: randomfp,
              type: config.net,
              sni: 'ZIZIFN-VlESS.PagES.DEV',// Set your host here -4
          });
          const vlessUrl = `vless://${config.id}@${config.add}:${config.port}?${queryParams.toString()}#${config.ps}`;
      ...
      for (const ip of shuffledIPListRE2) {
          ...
          const queryParams = new URLSearchParams({
              ...
              encryption: config.type,
              alpn: 'http/1.1',// preferred alpn type
              host: 'zizifn-vless.pages.dev',// Set your host here -5
              fp: randomfp,
              type: config.net,
              sni: 'ZIZIFN-VlESS.PagES.DEV',// Set your host here -6
          });
          const vlessUrl = `vless://${config.id}@${config.add}:${config.port}?${queryParams.toString()}#${config.ps}`;
